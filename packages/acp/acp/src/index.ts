...
/**
 * Compare one string to a candidate bearer token in constant time to avoid
 * timing oracles that could leak the token length or value.
 * @param candidate - the token from the client.
...
export interface AcpConfig {
  ...
  stream?: Stream
  /**
   * Optional bearer token required for all ACP requests. When set, the server
   * advertises `bearer` in `authMethods`; the client must call `authenticate`
   * with this token before any other method succeeds. When unset, the server
   * advertises no auth methods and accepts all requests — only safe for stdio
   * where the OS process boundary is the transport.
   */
  authToken?: string
}
...
export function apply(ctx: Context, config: AcpConfig): void {
  ...
  /** Whether the client has authenticated. Starts true when no token is configured. */
  let authenticated = config.authToken === undefined || config.authToken === ''
  let closed = false
  ...
  /** Reject any method that requires authentication when a token is configured and the client has not yet authenticated. */
  const assertAuthenticated = (): void => {
    if (!authenticated) throw internalError('authentication required')
  }
