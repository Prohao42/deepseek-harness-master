...
describe('automation-only ACP bridge', () => {
  ...
  it('advertises bearer auth when authToken is configured', async () => {
    harness = await makeBridgeHarness({ config: { authToken: 'test-token' } })
    const response = await harness.client.initialize({
      protocolVersion: PROTOCOL_VERSION,
      clientCapabilities: {},
    })
  ...
  it('rejects unauthenticated requests when authToken is configured', async () => {
    ...
    await expect(
      harness.client.newSession({ cwd: process.cwd(), mcpServers: [] }),
    ).rejects.toThrow('authentication required')
  })

  it('accepts requests after authenticate succeeds with bearer token', async () => {
    harness = await makeBridgeHarness({ config: { authToken: 'test-token' } })
    await harness.client.initialize({ protocolVersion: PROTOCOL_VERSION, clientCapabilities: {} })
    await harness.client.authenticate({
      methodId: 'bearer',
      _meta: { authorization: 'test-token' },
    })
