...
function parsePatchList(
  ...
  const schema = process.env.DSH_DISABLE_JS_CONFIG === '1'
    ? createSafeUserPatchesSchema()
    : userPatchesSchema
  let parsed: unknown
  try {
...
/**
 * Build a schema with `!!js` tags disabled for safe patch-list parsing.
 * Used when `DSH_DISABLE_JS_CONFIG=1` to prevent arbitrary JS execution via
 * YAML config files.
 */
function createSafeUserPatchesSchema(): yaml.Schema {
...
export function renderConfigDump(
  ...
  try {
    parsed = yaml.load(content, {
      schema: process.env.DSH_DISABLE_JS_CONFIG === '1' ? yaml.JSON_SCHEMA : entryListSchema,
    })
  } catch (error) {
